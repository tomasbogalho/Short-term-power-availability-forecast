...
def create_model(steps_before, steps_after, cnn, feature_count, units, layer, ft):
    """ 
    ...
    for l in range (0, len(sl)):
        ...
        for d in range (0, len(dtr)):
            ...
            for i in range (0, len(array)):
                ...
                #dt = dt[['AvailablePower']].copy()


                dt_val = dv[d].copy()

                dt_val = dt_val[finalfeatures].copy()
                #dt_val = dt_val[['AvailablePower']].copy()
                ...
                model = create_model_vanilla(steps_before = n_pre, feature_count = 8, units = U, layer = layer, intervals = False)
                ...
                for i in range(635, valX.shape[0], valX.shape[0]):
                ...
                validation_rmse_5_normalized, validation_rmse_10_normalized, validation_rmse_15_normalized, validation_mse_5_normalized, validation_mse_10_normalized, validation_mse_15_normalized, validation_mae_5_normalized, validation_mae_10_normalized, validation_mae_15_normalized =  printValidationErrorsNormalized(original_normalized, predicted_normalized)

                validation_rmse_5, validation_rmse_10, validation_rmse_15, validation_mse_5, validation_mse_10, validation_mse_15, validation_mae_5, validation_mae_10, validation_mae_15 = printValidationErrors(original, predicted)
                ...
                plt.xlabel('epoch')
                plt.legend()
                plt.savefig('./outputs/Images/' + model_name + '_Loss_')
                plt.show()

                with open('./outputs/ValidationProcess_' + dataset + '.csv', 'a', newline='') as file:
                ...
                model.save('./outputs/models/' + model_name + '.h5')
...
def encoder_decoder(layer, dtr, dv, cnn):
        ...
        for d in range (0, len(dtr)):
            ...
            for f in range (0, len(filters)):
                ...
                for i in range (0, len(array)):
                    ...
                    model_name = 'ED-' + layer + '-' + str(filters[f]) + '-' + str(array[i]) + '-'+ str(d+1) + '-'+ str(sl[l])
                    ...
                    print('creating model...')
                    model = create_model(steps_before = n_pre, steps_after = n_post, cnn = cnn, feature_count = 8, units = U, layer = layer, ft = fil)
                    ...
                    fig, ax = plt.subplots()
                    for i in range(0, valX.shape[0], valX.shape[0]):
